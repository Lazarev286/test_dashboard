...
from odoo.addons.lbs_dashboard.common_lib.ks_date_filter_selections import ks_get_date, ks_convert_into_utc, \
...
class LBSDashboardItems(models.Model):
    ...
    dashboard_board_template_id = fields.Many2one('lbs.dashboard_template', string="Dashboard Template")
    lbs_domain = fields.Char(string="Domain", help="Define conditions for filter. ")

    lbs_model_id_2 = fields.Many2one('ir.model', string='Kpi Model',
                                    domain="[('access_ids','!=',False),('transient','=',False),"
                                           "('model','not ilike','base_import%'),('model','not ilike','ir.%'),"
                                           "('model','not ilike','web_editor.%'),('model','not ilike','web_tour.%'),"
                                           "('model','!=','mail.thread'),('model','not ilike','ks_dash%'), ('model','not ilike','ks_to%')]")

    lbs_model_name_2 = fields.Char(related='lbs_model_id_2.model', string="Kpi Model Name")
    ...
    lbs_icon = fields.Binary(string="Upload Icon", attachment=True)
    default_icon = fields.Char(string="Icon", default="bar-chart", help='Select the icon to be displayed. ')
    default_icon_color = fields.Char(default="#ffffff,0.99", string="Icon Color",
                                        help='Select the icon to be displayed. ')
    icon_select = fields.Char(string="Icon Option", default="Default", help='Choose the Icon option. ')
    lbs_font_color = fields.Char(default="#ffffff,0.99", string="Font Color", help='Select the font color. ')
    dashboard_item_theme = fields.Char(string="Theme", default="white",
                                          help='Select the color theme for the display. ')
    lbs_layout = fields.Selection([('layout1', 'Layout 1'),
                                  ('layout2', 'Layout 2'),
                                  ('layout3', 'Layout 3'),
                                  ('layout4', 'Layout 4'),
                                  ('layout5', 'Layout 5'),
    ...
    lbs_preview = fields.Integer(default=1, string="Preview")
    lbs_model_name = fields.Char(related='model_id.model', string="Model Name")

    lbs_record_count_type_2 = fields.Selection([('count', 'Count'),
                                               ('sum', 'Sum'),
                                               ('average', 'Average')], string="Kpi Record Type", default="sum")
    record_field_2 = fields.Many2one('ir.model.fields',
                                        domain="[('model_id','=',lbs_model_id_2),('name','!=','id'),('name','!=','sequence'),('store','=',True),"
                                               "'|','|',('ttype','=','integer'),('ttype','=','float'),"
                                               "('ttype','=','monetary')]",
                                        string="Kpi Record Field")
    record_count_2 = fields.Float(string="KPI Record Count", readonly=True, compute='ks_get_record_count_2',
                                     compute_sudo=False)
    record_count_type = fields.Selection([('count', 'Count'),
                                             ('sum', 'Sum'),
    ...
    lbs_record_count = fields.Float(string="Record Count", compute='ks_get_record_count', readonly=True,
                                   compute_sudo=False)
    lbs_record_field = fields.Many2one('ir.model.fields',
                                      domain="[('model_id','=',model_id),('name','!=','id'),('store','=',True),'|',"
                                             "'|',('ttype','=','integer'),('ttype','=','float'),"
                                             "('ttype','=','monetary')]",
                                      string="Record Field")
    record_data_limit_visibility = fields.Boolean(string="Record Limit Data Visibility",
                                                     help="To enable the record data limit field")

    # Date Filter Fields
    # Condition to tell if date filter is applied or not
    lbs_isDateFilterApplied = fields.Boolean(default=False)
    ...
    date_filter_field = fields.Many2one('ir.model.fields',
                                           domain="[('model_id','=',model_id),('store','=',True),'|',('ttype','=','date'),"
                                                  "('ttype','=','datetime')]",
    ...
    date_filter_field_2 = fields.Many2one('ir.model.fields',
                                             domain="[('model_id','=',lbs_model_id_2),('store','=',True),'|',('ttype','=','date'),"
                                                    "('ttype','=','datetime')]",
                                             string="Kpi Date Filter Field")

    item_start_date_2 = fields.Datetime(string="Kpi Start Date")
    item_end_date_2 = fields.Datetime(string="Kpi End Date")

    lbs_domain_2 = fields.Char(string="Kpi Domain")
    lbs_domain_2_temp = fields.Char(string="Kpi Domain Substitute")
    ...
    previous_period = fields.Boolean(string=" Compare With Previous Period ", help='Checkbox to show comparison between the data of present day and the previous selected period. ')

    # ------------------------ Pro Fields --------------------
    dashboard_lbs_board_id = fields.Many2one('lbs.dashboard', string="Dashboard",
                                                  default=lambda self: self._context[
                                                      'ks_dashboard_id'] if 'ks_dashboard_id' in self._context
                                                  else False)

    # Chart related fields
    dashboard_item_type = fields.Selection([('ks_tile', 'Tile'),
                                               ...
                                               ('ks_kpi', 'KPI'),
                                               ('ks_to_do', 'To Do')
                                               ], default=lambda self: self._context.get('dashboard_item_type',
                                                                                         'ks_tile'), required=True,
                                              string="Dashboard Item Type",
                                              help="Select the required type of dashboard to display. ")
    chart_groupby_type = fields.Char(compute='get_chart_groupby_type', compute_sudo=False)
    chart_sub_groupby_type = fields.Char(compute='get_chart_sub_groupby_type', compute_sudo=False)
    lbs_chart_relation_groupby = fields.Many2one('ir.model.fields',
    ...
    ks_chart_cumulative_field = fields.Many2many('ir.model.fields', 'ks_dn_cumulative_measure_field_rel',
                                                 ...
                                                 'cumulative_field_id',
                                                 domain="[('model_id','=',model_id),('name','!=','id'),('name',"
                                                        "'!=','sequence'), "
                                                        "('store','=',True),'|','|',"
                                                        "('ttype','=','integer'),('ttype','=','float'),"
                                                        "('ttype','=','monetary')]",
    ...
    ks_sort_by_field = fields.Many2one('ir.model.fields',
                                       domain="[('model_id','=',model_id),('name','!=','id'),('name','!=','sequence'),('store','=',True),"
    ...
    ks_goal_lines = fields.One2many('lbs.dashboard_item_goal', 'ks_dashboard_item', string="Target Lines")
    ...
    ks_action_lines = fields.One2many('lbs.dashboard_item_action', 'ks_dashboard_item_id', string="Action Lines")

    ks_actions = fields.Many2one('ir.actions.act_window', domain="[('res_model','=',lbs_model_name)]",
    ...
    ks_update_items_data = fields.Selection([
        ...
        ('300000', '5 minute'),
        ('600000', '10 minute'),
    ], string="Item Update Interval", default=lambda self: self._context.get('set_interval', False),
    ...
    ks_auto_update_type = fields.Selection(
        ...
        default=lambda self: 'ks_update_interval' if self._context.get('set_interval', False) else False,
    ...
    ks_multiplier_lines = fields.One2many('lbs.dashboard_items_multiplier', 'ks_dashboard_item_id',
    ...
    @api.onchange('item_start_date', 'item_end_date')
    def ks_item_date_validation(self):
        for rec in self:
            if rec.item_start_date and rec.item_end_date:
                if rec.item_start_date > rec.item_end_date:
                    raise ValidationError(_('Start date must be less than end date'))

    @api.onchange('item_start_date_2', 'item_end_date_2')
    def ks_item_date_validation_2(self):
        for rec in self:
            if rec.item_start_date_2 and rec.item_end_date_2:
                if rec.item_start_date_2 > rec.item_end_date_2:
                    raise ValidationError(_('Start date must be less than end date'))

    @api.depends('dashboard_item_type')
    def _ks_compute_precision_digits(self):
        for rec in self:
            try:
                precision_digits = self.sudo().env.ref('lbs_dashboard.ks_dashboard_ninja_precision')
    ...
    # default = lambda self: self.sudo().env.ref('lbs_dashboard.ks_dashboard_ninja_precision')
    ...
    def _ks_compute_multiplier_lines(self):
        for rec in self:
            ...
            if rec.ks_multiplier_active:
                if rec.dashboard_item_type == 'ks_list_view' and rec.ks_list_view_type == 'grouped':
                ...
                for ks_chart_measure_field in ks_chart_measure_fields:
                    ...
                    ks_line = self.env['lbs.dashboard_items_multiplier'].create(ks_dict)
    ...
    @api.onchange('record_data_limit_visibility')
    def ks_on_change_record_data_visibility(self):
        for rec in self:
            if not rec.record_data_limit_visibility:
    ...
    def ks_onchange_fill_temporal(self):
        if self.ks_fill_temporal:
            self.ks_sort_by_field = self.lbs_chart_relation_groupby.id
    ...
    def ks_date_target(self):
        for rec in self:
            ...
            if rec.lbs_chart_relation_groupby.ttype == 'date' and rec.ks_chart_date_groupby in ('minute', 'hour'):
                raise ValidationError(_('Groupby field: {} cannot be aggregated by {}').format(
                    rec.lbs_chart_relation_groupby.display_name, rec.ks_chart_date_groupby))
    ...
    def copy_data(self, default=None):
        ...
        return super(LBSDashboardItems, self).copy_data(default)

    def copy(self, default=None):
        default = default or {}
        res = super(LBSDashboardItems, self).copy(default)
    ...
    def create(self, values):
        ...
        return super(LBSDashboardItems, self).create(
            values)
    ...
    def write(self, values):
        ...
        return super(LBSDashboardItems, self).write(
            values)

    @api.onchange('lbs_layout')
    def layout_four_font_change(self):
        if self.dashboard_item_theme != "white":
            if self.lbs_layout == 'layout4':
                self.lbs_font_color = self.background_color
                self.default_icon_color = "#ffffff,0.99"
            elif self.lbs_layout == 'layout6':
                self.lbs_font_color = "#ffffff,0.99"
                self.default_icon_color = self.ks_get_dark_color(self.background_color.split(',')[0],
                                                                    self.background_color.split(',')[1])
            else:
                self.default_icon_color = "#ffffff,0.99"
                self.lbs_font_color = "#ffffff,0.99"
        else:
            if self.lbs_layout == 'layout4':
                ...
                self.lbs_font_color = "#00000,0.99"
                self.default_icon_color = "#00000,0.99"
    ...
    def make_record_field_empty(self):
        for rec in self:
            rec.lbs_record_field = False
            rec.lbs_domain = False
            rec.date_filter_field = False
            # To show "created on" by default on date filter field on model select.
            if rec.model_id:
                datetime_field_list = rec.date_filter_field.search(
                    [('model_id', '=', rec.model_id.id), '|', ('ttype', '=', 'date'),
                     ('ttype', '=', 'datetime')]).read(['id', 'name'])
                for field in datetime_field_list:
                    if field['name'] == 'create_date':
                        rec.date_filter_field = field['id']
            else:
                rec.date_filter_field = False
            # Pro
            rec.lbs_record_field = False
            ...
            rec.lbs_chart_relation_groupby = False
    ...
    @api.onchange('lbs_record_count', 'lbs_layout', 'name', 'model_id', 'lbs_domain', 'icon_select',
                  'default_icon', 'lbs_icon',
                  'background_color', 'lbs_font_color', 'default_icon_color')
    def ks_preview_update(self):
        self.lbs_preview += 1

    @api.onchange('dashboard_item_theme')
    def change_dashboard_item_theme(self):
        if self.dashboard_item_theme == "red":
            self.background_color = "#d9534f,0.99"
            self.default_icon_color = "#ffffff,0.99"
            self.lbs_font_color = "#ffffff,0.99"
            self.ks_button_color = "#000000,0.99"
        elif self.dashboard_item_theme == "blue":
            self.background_color = "#337ab7,0.99"
            self.default_icon_color = "#ffffff,0.99"
            self.lbs_font_color = "#ffffff,0.99"
            self.ks_button_color = "#000000,0.99"
        elif self.dashboard_item_theme == "yellow":
            self.background_color = "#f0ad4e,0.99"
            self.default_icon_color = "#ffffff,0.99"
            self.lbs_font_color = "#ffffff,0.99"
            self.ks_button_color = "#000000,0.99"
        elif self.dashboard_item_theme == "green":
            self.background_color = "#5cb85c,0.99"
            self.default_icon_color = "#ffffff,0.99"
            self.lbs_font_color = "#ffffff,0.99"
            self.ks_button_color = "#000000,0.99"
        elif self.dashboard_item_theme == "white":
            if self.lbs_layout == 'layout4':
                self.background_color = "#00000,0.99"
                self.default_icon_color = "#ffffff,0.99"
                self.ks_button_color = "#000000,0.99"
                ...
                self.lbs_font_color = "#000000,0.99"
                self.ks_button_color = "#000000,0.99"
    ...
    @api.depends('record_count_type', 'model_id', 'lbs_domain', 'lbs_record_field', 'date_filter_field',
                 'item_end_date', 'item_start_date', 'ks_compare_period', 'ks_year_period',
                 'dashboard_item_type', 'ks_domain_extension', 'ks_data_format')
    def ks_get_record_count(self):
        for rec in self:
            rec.lbs_record_count = rec._ksGetRecordCount(domain=[])

    def _ksGetRecordCount(self, domain=[]):
        rec = self
        if rec.record_count_type == 'count' or rec.dashboard_item_type == 'ks_list_view':
            lbs_record_count = rec.ks_fetch_model_data(rec.lbs_model_name, rec.lbs_domain, 'search_count', rec, domain)
        elif rec.record_count_type in ['sum',
                                          'average'] and rec.lbs_record_field and rec.dashboard_item_type != 'ks_list_view':
            ks_records_grouped_data = rec.ks_fetch_model_data(rec.lbs_model_name, rec.lbs_domain, 'read_group', rec,
                                                              domain)
            if ks_records_grouped_data and len(ks_records_grouped_data) > 0:
                ks_records_grouped_data = ks_records_grouped_data[0]
                if rec.record_count_type == 'sum' and ks_records_grouped_data.get('__count', False) and (
                        ks_records_grouped_data.get(rec.lbs_record_field.name)):
                    lbs_record_count = ks_records_grouped_data.get(rec.lbs_record_field.name, 0)
                elif rec.record_count_type == 'average' and ks_records_grouped_data.get(
                        '__count', False) and (ks_records_grouped_data.get(rec.lbs_record_field.name)):
                    lbs_record_count = ks_records_grouped_data.get(rec.lbs_record_field.name,
            ...
            lbs_record_count = 0
        return lbs_record_count

    # Writing separate function to fetch dashboard item data
    def ks_fetch_model_data(self, lbs_model_name, lbs_domain, ks_func, rec, domain=[]):
        data = 0
        try:
            if lbs_domain and lbs_domain != '[]' and lbs_model_name:
                proper_domain = self.ks_convert_into_proper_domain(lbs_domain, rec, domain)
                if ks_func == 'search_count':
                    data = self.env[lbs_model_name].search_count(proper_domain)
                elif ks_func == 'read_group':
                    data = self.env[lbs_model_name].read_group(proper_domain, [rec.lbs_record_field.name], [], lazy=False)
            elif lbs_model_name:
    ...
    def ks_convert_into_proper_domain(self, lbs_domain, rec, domain=[]):
        ...
        if rec.date_filter_field:
            if not rec.date_filter_selection or rec.date_filter_selection == "l_none":
                ...
                if selected_start_date and selected_end_date and rec.date_filter_field.ttype == 'datetime' and not ks_is_def_custom_filter:
                ...
                if selected_start_date and selected_end_date and rec.date_filter_field.ttype == 'date' and ks_is_def_custom_filter:
                ...
                if rec.date_filter_selection and rec.date_filter_selection != 'l_custom':
                    ks_date_data = ks_get_date(rec.date_filter_selection, self, rec.date_filter_field.ttype)
                    selected_start_date = ks_date_data["selected_start_date"]
                    selected_end_date = ks_date_data["selected_end_date"]
                else:
                    selected_start_date = False
                    selected_end_date = False
                    if rec.item_start_date or rec.item_end_date:
                        selected_start_date = rec.item_start_date
                        selected_end_date = rec.item_end_date
                        if rec.date_filter_field.ttype == 'date' and rec.item_start_date and rec.item_end_date:
                            ks_timezone = self._context.get('tz') or self.env.user.tz
                            selected_start_date = ks_convert_into_local(rec.item_start_date, ks_timezone)
                            selected_end_date = ks_convert_into_local(rec.item_end_date, ks_timezone)
                ...
                if selected_start_date and selected_end_date:
                    if rec.ks_compare_period:
                        ...
                        if rec.ks_compare_period > 0:
                            ...
                            if rec.date_filter_field.ttype == "date" and rec.date_filter_selection == 'l_day':
                    ...
                    if rec.ks_year_period and rec.ks_year_period != 0 and rec.dashboard_item_type:
                        abs_year_period = abs(rec.ks_year_period)
                        sign_yp = rec.ks_year_period / abs_year_period
                        if abs_year_period > 100:
                            abs_year_period = 100
                        date_field_name = rec.date_filter_field.name
                        ...
                        ks_date_domain = [(rec.date_filter_field.name, ">=", selected_start_date),
                                          (rec.date_filter_field.name, "<=", selected_end_date)]
                elif selected_start_date and not selected_end_date:
                    ...
                    selected_end_date = fields.datetime.strftime(selected_end_date, DEFAULT_SERVER_DATETIME_FORMAT)
                    ks_date_domain = [(rec.date_filter_field.name, "<=", selected_end_date)]
        else:
            ks_date_domain = []

        proper_domain = safe_eval(lbs_domain) if lbs_domain else []
        if ks_date_domain:
    ...
    def ks_convert_domain_extension(self, ks_extensiom_domain, rec):
        ...
        lbs_domain = eval(ks_extensiom_domain)
        return lbs_domain
    ...
    def ks_onchange_domain_extension(self):
        if self.ks_domain_extension:
            ...
            try:
                ...
                self.env[self.lbs_model_name].search_count(safe_eval(ks_domain_extension))
            except Exception:
    ...
    @api.depends('lbs_chart_relation_groupby')
    def get_chart_groupby_type(self):
        for rec in self:
            if rec.lbs_chart_relation_groupby.ttype == 'datetime' or rec.lbs_chart_relation_groupby.ttype == 'date':
                rec.chart_groupby_type = 'date_type'
            elif rec.lbs_chart_relation_groupby.ttype == 'many2one':
                rec.chart_groupby_type = 'relational_type'
                rec.ks_chart_date_groupby = False
            elif rec.lbs_chart_relation_groupby.ttype == 'selection':
                rec.chart_groupby_type = 'selection'
                rec.ks_chart_date_groupby = False
            else:
                rec.chart_groupby_type = 'other'

    @api.onchange('lbs_chart_relation_groupby')
    def ks_empty_sub_group_by(self):
        for rec in self:
            if not rec.lbs_chart_relation_groupby or rec.chart_groupby_type == "date_type" \
                    and not rec.ks_chart_date_groupby:
                rec.ks_chart_relation_sub_groupby = False
                rec.ks_chart_date_sub_groupby = False
            if not (rec.lbs_chart_relation_groupby.ttype == 'datetime' or \
                    rec.lbs_chart_relation_groupby.ttype == 'date'):
                rec.ks_goal_lines = False
    ...
    def get_chart_sub_groupby_type(self):
        for rec in self:
            if rec.ks_chart_relation_sub_groupby.ttype == 'datetime' or \
                ...
                rec.chart_sub_groupby_type = 'selection'

            else:
                rec.chart_sub_groupby_type = 'other'

    @api.depends('ks_chart_measure_field', 'ks_chart_cumulative_field', 'lbs_chart_relation_groupby',
                 'ks_chart_date_groupby', 'lbs_domain',
                 'dashboard_item_type', 'model_id', 'ks_sort_by_field', 'ks_sort_by_order',
                 ...
                 'ks_chart_date_sub_groupby', 'date_filter_field', 'item_start_date', 'item_end_date',
    ...
    def _ks_get_chart_data(self, domain=[]):
        rec = self
        if rec.dashboard_item_type and rec.dashboard_item_type != 'ks_tile' and \
                rec.dashboard_item_type != 'ks_list_view' and rec.model_id and rec.ks_chart_data_count_type:
            ...
            if rec.ks_chart_data_count_type == "count":
                ...
                if rec.dashboard_item_type == 'ks_bar_chart':
            ...
            ks_chart_groupby_relation_field = rec.lbs_chart_relation_groupby.name
            ks_chart_domain = self.ks_convert_into_proper_domain(rec.lbs_domain, rec, domain)
            ...
            if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                ...
                if rec.lbs_chart_relation_groupby.ttype == 'date' and rec.ks_chart_date_groupby in (
                        'minute', 'hour'):
                    raise ValidationError(_('Groupby field: {} cannot be aggregated by {}').format(
                        rec.lbs_chart_relation_groupby.display_name, rec.ks_chart_date_groupby))
                    lbs_chart_relation_groupby = 'day'
                elif rec.ks_chart_date_groupby == 'month_year':
                    ks_chart_date_groupby = 'month'
                else:
                    ks_chart_date_groupby = rec.ks_chart_date_groupby

                if (rec.chart_groupby_type == 'date_type' and rec.ks_chart_date_groupby) or \
                        rec.chart_groupby_type != 'date_type':
                    ks_chart_data = rec.ks_fetch_chart_data(rec.lbs_model_name, ks_chart_domain,
                                                            ks_chart_measure_field_with_type,
                                                            ks_chart_measure_field_with_type_2,
                                                            ks_chart_measure_field,
                                                            ks_chart_measure_field_2,
                                                            ...
                                                            rec.chart_groupby_type, orderby, limit,
                                                            rec.ks_chart_data_count_type,
                                                            ks_chart_measure_field_ids,
                                                            ks_chart_measure_field_2_ids,
                                                            rec.lbs_chart_relation_groupby.id, ks_chart_data)

                    if rec.chart_groupby_type == 'date_type' and rec.ks_goal_enable and rec.dashboard_item_type in [
                        'ks_bar_chart', 'ks_horizontalBar_chart', 'ks_line_chart',
                        'ks_area_chart'] and rec.chart_groupby_type == "date_type":
                        ...
                        if rec.date_filter_selection == "l_none":
                            ...
                            if rec.date_filter_selection == "l_custom":
                                selected_start_date = rec.item_start_date
                                selected_end_date = rec.item_end_date
                            else:
                                ks_date_data = ks_get_date(rec.date_filter_selection, self,
                                                           rec.date_filter_field.ttype)
                                selected_start_date = ks_date_data["selected_start_date"]
                                selected_end_date = ks_date_data["selected_end_date"]
                        ...
                        ks_date_data = rec.ks_get_start_end_date(rec.lbs_model_name, ks_chart_groupby_relation_field,
                                                                 rec.lbs_chart_relation_groupby.ttype,
                                                                 ks_chart_domain,
                                                                 ks_goal_domain)
                        ...
                        ks_goal_records = self.env['lbs.dashboard_item_goal'].read_group(
            ...
            elif rec.ks_chart_relation_sub_groupby and ((rec.chart_sub_groupby_type == 'relational_type') or
                                                        (rec.chart_sub_groupby_type == 'selection') or
                                                        (rec.chart_sub_groupby_type == 'date_type' and
                ...
                if len(ks_chart_measure_field) != 0 or rec.ks_chart_data_count_type == 'count':
                    if rec.chart_groupby_type == 'date_type' and ks_chart_date_groupby:
                        ks_chart_group = rec.lbs_chart_relation_groupby.name + ":" + ks_chart_date_groupby
                    else:
                        ks_chart_group = rec.lbs_chart_relation_groupby.name

                    if rec.chart_sub_groupby_type == 'date_type' and rec.ks_chart_date_sub_groupby:
                    ...
                    try:
                        ks_chart_record = self.env[rec.lbs_model_name].read_group(ks_chart_domain,
                                                                                 list(set(
                                                                                     ks_chart_measure_field_with_type +
                                                                                     ks_chart_measure_field_with_type_2 +
                                                                                     [
                    ...
                    for res in ks_chart_record:
                        ...
                        if res.get(ks_chart_groupby_relation_fields[0], False):
                            if rec.chart_groupby_type == 'date_type':
                                # x-axis modification
                            ...
                            if rec.ks_chart_data_count_type != 'count':
                                for ress in rec.ks_chart_measure_field:
                                    if rec.chart_sub_groupby_type == 'date_type':
                                    ...
                                    elif rec.chart_sub_groupby_type == 'selection':
                                        if res[ks_chart_groupby_relation_fields[1]] is not False:
                                            selection = res[ks_chart_groupby_relation_fields[1]]
                                            labels.append(dict(self.env[rec.lbs_model_name].fields_get(
                                            ...
                                            labels.append(str(res[ks_chart_groupby_relation_fields[1]]))
                                    elif rec.chart_sub_groupby_type == 'relational_type':
                                        if res[ks_chart_groupby_relation_fields[1]] is not False:
                                            ...
                                            labels.append(str(res[ks_chart_groupby_relation_fields[1]])
                                                          + " " + ress.field_description)
                                    elif rec.chart_sub_groupby_type == 'other':
                                        if res[ks_chart_groupby_relation_fields[1]] is not False:
                                            labels.append(str(res[ks_chart_groupby_relation_fields[1]])
                                ...
                                if rec.ks_chart_measure_field_2 and rec.dashboard_item_type == 'ks_bar_chart':
                                    for ress in rec.ks_chart_measure_field_2:
                                        if rec.chart_sub_groupby_type == 'date_type':
                                            ...
                                            selection = res[ks_chart_groupby_relation_fields[1]]
                                            labels_2.append(dict(self.env[rec.lbs_model_name].fields_get(
                                                allfields=[ks_chart_groupby_relation_fields[1]])
                    ...
                    if rec.ks_chart_relation_sub_groupby.name == rec.lbs_chart_relation_groupby.name == rec.ks_sort_by_field.name:
                        ks_data = rec.ks_sort_sub_group_by_records(ks_data, rec.chart_groupby_type,
                    ...
                    if len(ks_data) != 0:
                        ...
                        if rec.ks_goal_enable and rec.ks_standard_goal_value and rec.dashboard_item_type in [
                            ...
                            if rec.ks_goal_bar_line and rec.dashboard_item_type != 'ks_horizontalBar_chart':
                                ks_goal_datasets['type'] = 'line'
                                ks_chart_data['datasets'].insert(0, ks_goal_datasets)
                            else:
                                ks_chart_data['datasets'].append(ks_goal_datasets)
    ...
    @api.depends('lbs_domain', 'dashboard_item_type', 'ks_pagination_limit', 'model_id', 'ks_sort_by_field',
                 ...
                 'chart_groupby_type', 'ks_chart_date_groupby', 'date_filter_field', 'item_end_date',
                 'item_start_date', 'ks_compare_period', 'ks_year_period', 'ks_list_target_deviation_field',
    ...
    def _ksGetListViewData(self, domain=[]):
        rec = self
        if rec.ks_list_view_type and rec.dashboard_item_type and rec.dashboard_item_type == 'ks_list_view' \
                and rec.model_id:
    ...
    def get_list_view_record(self, orderid, sort_order, ks_chart_domain, ksoffset=0,
        ...
        ks_list_view_data = {'label': [], 'fields': [], 'fields_type': [],
                             ...
                             'data_rows': [], 'model': self.lbs_model_name}
        ...
        if self.ks_list_view_type == "ungrouped":
            if self.ks_list_view_fields:
                ...
                and self.lbs_chart_relation_groupby:
            ks_list_fields = []

            if self.chart_groupby_type == 'relational_type':
                ks_list_view_data['list_view_type'] = 'relational_type'
                ks_list_view_data['groupby'] = self.lbs_chart_relation_groupby.name
                ks_list_fields.append(self.lbs_chart_relation_groupby.name)
                ks_list_view_data['fields'].append(self.lbs_chart_relation_groupby.ids[0])
                ks_list_view_data['fields_type'].append(self.lbs_chart_relation_groupby.ttype)
                ks_list_view_data['store'].append(self.lbs_chart_relation_groupby.store)
                ks_list_view_data['label'].append(self.lbs_chart_relation_groupby.field_description)
                ...
                try:
                    ks_list_view_records = self.env[self.lbs_model_name]. \
                    read_group(ks_chart_domain, ks_list_fields, [self.lbs_chart_relation_groupby.name],
                ...
                for res in ks_list_view_records:
                    if all(list_fields in res for list_fields in ks_list_fields) \
                            and res[self.lbs_chart_relation_groupby.name]:
                        counter = 0
                        data_row = {'id': res[self.lbs_chart_relation_groupby.name][0], 'data': [],
                        ...
                        for field_rec in ks_list_fields:
                            ...
                            counter += 1
                            data_row['ks_column_type'].append(self.lbs_chart_relation_groupby.ttype)
                        ks_list_view_data['data_rows'].append(data_row)

            elif self.chart_groupby_type == 'date_type' and self.ks_chart_date_groupby:
                ...
                if self.ks_chart_date_groupby == 'month_year':
                    ks_chart_date_groupby = 'month'
                ks_list_view_data[
                    'groupby'] = self.lbs_chart_relation_groupby.name + ':' + ks_chart_date_groupby
                ks_list_field.append(self.lbs_chart_relation_groupby.name)
                ks_list_fields.append(self.lbs_chart_relation_groupby.name + ':' + ks_chart_date_groupby)
                ks_list_view_data['label'].append(
                    self.lbs_chart_relation_groupby.field_description + ' : ' + ks_chart_date_groupby
                    .capitalize())
                ks_list_view_data['fields'].append(self.lbs_chart_relation_groupby.ids[0])
                ks_list_view_data['fields_type'].append(self.lbs_chart_relation_groupby.ttype)
                ks_list_view_data['store'].append(self.lbs_chart_relation_groupby.store)
                ...
                try:
                    ...
                    read_group(ks_chart_domain, ks_list_field + list_target_deviation_field,
                               [self.lbs_chart_relation_groupby.name + ':' + ks_chart_date_groupby],
                               orderby=orderby, limit=limit, offset=ksoffset, lazy=False)
                except Exception as E:
                    ks_list_view_records = []
            ...
            elif self.chart_groupby_type == 'selection':
                ks_list_view_data['list_view_type'] = 'selection'
                ks_list_view_data['groupby'] = self.lbs_chart_relation_groupby.name
                ks_list_view_data['fields'].append(self.lbs_chart_relation_groupby.ids[0])
                ks_list_view_data['fields_type'].append(self.lbs_chart_relation_groupby.ttype)
                ks_list_view_data['store'].append(self.lbs_chart_relation_groupby.store)
                ks_selection_field = self.lbs_chart_relation_groupby.name
                ks_list_view_data['label'].append(self.lbs_chart_relation_groupby.field_description)
                for res in self.ks_list_view_group_fields:
                    ks_list_fields.append(res.name)
                    ks_list_view_data['label'].append(res.field_description)
                    ...
                    if all(list_fields in res for list_fields in ks_list_fields):
                        ...
                        if res[ks_selection_field]:
                            data_row['data'].append(dict(
                                self.env[self.lbs_model_name].fields_get(allfields=ks_selection_field)
            ...
            elif self.chart_groupby_type == 'other':
                ...
                ks_list_view_data['label'].append(self.lbs_chart_relation_groupby.field_description)
                for res in self.ks_list_view_group_fields:
                    if res.name != self.lbs_chart_relation_groupby.name:
                        ks_list_fields.append(res.name)
                        ks_list_view_data['label'].append(res.field_description)
                        ks_list_view_data['fields'].append(res.ids[0])
                        ks_list_view_data['fields_type'].append(res.ttype)
                    ...
                    if all(list_fields in res for list_fields in ks_list_fields):
                        ...
                        for field_rec in ks_list_fields:
                            if counter == 0:
                                ...
                                if self.lbs_chart_relation_groupby.name == field_rec:
    ...
    def get_target_list_view_data(self, ks_list_view_records, rec, ks_list_fields, ks_group_by,
        ...
        if rec.date_filter_selection_2 == "l_none":
            selected_start_date = rec._context.get('ksDateFilterStartDate', False)
            selected_end_date = rec._context.get('ksDateFilterEndDate', False)
        else:
            selected_start_date = rec.item_start_date
        ...
        ks_date_data = rec.ks_get_start_end_date(rec.lbs_model_name, rec.lbs_chart_relation_groupby.name,
                                                 rec.lbs_chart_relation_groupby.ttype,
                                                 ks_chart_domain,
                                                 ks_goal_domain)
        ...
        if rec.ks_goal_lines and len(rec.ks_goal_lines) != 0:
            ...
            for label in ks_list_labels_dates:
                ...
                data_rows['ks_column_type'].append(self.lbs_chart_relation_groupby.ttype)
    ...
    def ks_fetch_list_view_data(self, rec, ks_chart_domain, limit=15, offset=0, ks_export_all=False, initial_count=0):
        ...
        try:
            ks_list_view_records = self.env[self.lbs_model_name].search_read(ks_chart_domain,
                                                                            ks_list_view_fields,
        ...
        for res in ks_list_view_records:
            ...
            for field_rec in ks_list_view_fields:
                if type(res[field_rec]) == fields.datetime or type(res[field_rec]) == fields.date:
                    ...
                    res[field_rec] = dict(self.env[rec.lbs_model_name].fields_get(allfields=[field_rec])
    ...
    @api.onchange('dashboard_item_type')
    def set_color_palette(self):
        for rec in self:
            if rec.dashboard_item_type == "ks_bar_chart" or rec.dashboard_item_type == "ks_horizontalBar_chart" \
                    or rec.dashboard_item_type == "ks_line_chart" or rec.dashboard_item_type == "ks_area_chart":
                rec.ks_chart_item_color = "cool"
            else:
                rec.ks_chart_item_color = "default"
            if rec.dashboard_item_type == 'ks_kpi' or rec.dashboard_item_type == 'ks_tile':
                rec.ks_data_calculation_type = 'custom'
            if rec.dashboard_item_type != "ks_bar_chart":
                rec.ks_chart_cumulative_field = False
                rec.ks_chart_cumulative = False
            rec.ks_multiplier_active = False
            rec.lbs_model_id_2 = False
            if rec.dashboard_item_type == 'ks_to_do':
                rec.lbs_model_id_2 = False
                rec.model_id = False
    ...
    def ks_set_date_filter(self):
        for rec in self:
            if (not rec.date_filter_selection) or rec.date_filter_selection == "l_none":
                rec.item_start_date = rec.item_end_date = False
            elif rec.date_filter_selection != 'l_custom':
                ks_date_data = ks_get_date(rec.date_filter_selection, self, rec.date_filter_field.ttype)
                rec.item_start_date = ks_date_data["selected_start_date"]
                rec.item_end_date = ks_date_data["selected_end_date"]

    @api.depends('dashboard_item_type', 'ks_goal_enable', 'ks_standard_goal_value', 'lbs_record_count',
                 'record_count_2', 'previous_period', 'ks_compare_period', 'ks_year_period',
    ...
    def _ksGetKpiData(self, domain1=[], domain2=[]):
        rec = self
        if rec.dashboard_item_type and rec.dashboard_item_type == 'ks_kpi' and rec.model_id:
            ks_kpi_data = []
            lbs_record_count = 0.0
            ks_kpi_data_model_1 = {}
            lbs_record_count = rec._ksGetRecordCount(domain1)
            ks_kpi_data_model_1['model'] = rec.lbs_model_name
            ks_kpi_data_model_1['record_field'] = rec.lbs_record_field.field_description
            ks_kpi_data_model_1['record_data'] = lbs_record_count

            if rec.ks_goal_enable:
                ks_kpi_data_model_1['target'] = rec.ks_standard_goal_value
            ks_kpi_data.append(ks_kpi_data_model_1)

            if rec.previous_period:
                ks_previous_period_data = rec.ks_get_previous_period_data(rec)
                ks_kpi_data_model_1['previous_period'] = ks_previous_period_data

            if rec.lbs_model_id_2 and rec.lbs_record_count_type_2:
                ks_kpi_data_model_2 = {}
                ks_kpi_data_model_2['model'] = rec.lbs_model_name_2
                ks_kpi_data_model_2[
                    'record_field'] = 'count' if rec.lbs_record_count_type_2 == 'count' else \
                    rec.record_field_2.field_description
    ...
    def ks_get_previous_period_data(self, rec):
        ...
        ks_date_data = False
        if rec.date_filter_selection == "l_none":
            date_filter_selection = rec.dashboard_lbs_board_id.date_filter_selection
        else:
            date_filter_selection = rec.date_filter_selection
            previous_period = switcher.get(date_filter_selection, False)
        if previous_period:
            ks_date_data = ks_get_date(previous_period, self, rec.date_filter_field.ttype)

        if (ks_date_data):
            previous_period_start_date = ks_date_data["selected_start_date"]
            previous_period_end_date = ks_date_data["selected_end_date"]
            proper_domain = rec.ks_get_previous_period_domain(rec.lbs_domain, previous_period_start_date,
                                                              previous_period_end_date, rec.date_filter_field)
            lbs_record_count = 0.0

            if rec.record_count_type == 'count':
                lbs_record_count = 0
                try:
                    lbs_record_count = self.env[rec.lbs_model_name].search_count(proper_domain)
                except Exception as E:
                    lbs_record_count = 0
                return lbs_record_count

            elif rec.lbs_record_field:
                try:
                    data = \
                        self.env[rec.lbs_model_name].read_group(proper_domain, [rec.lbs_record_field.name], [], lazy=False)[0]
                except Exception as E:
                    data = {}
                if rec.record_count_type == 'sum':
                    return data.get(rec.lbs_record_field.name, 0) if data.get('__count', False) and (
                        data.get(rec.lbs_record_field.name)) else 0
                else:
                    return data.get(rec.lbs_record_field.name, 0) / data.get('__count', 1) \
                        if data.get('__count', False) and (data.get(rec.lbs_record_field.name)) else 0
            else:
                return False
        else:
            return False

    def ks_get_previous_period_domain(self, lbs_domain, ks_start_date, ks_end_date, date_filter_field):
        if lbs_domain and "%UID" in lbs_domain:
            lbs_domain = lbs_domain.replace('"%UID"', str(self.env.user.id))
        if lbs_domain:
            # try:
            proper_domain = safe_eval(lbs_domain)
    ...
    @api.depends('lbs_domain_2', 'lbs_model_id_2', 'record_field_2', 'lbs_record_count_type_2', 'item_start_date_2',
                 'date_filter_selection_2', 'lbs_record_count_type_2', 'ks_compare_period_2', 'ks_year_period_2')
    def ks_get_record_count_2(self):
        for rec in self:
            rec.record_count_2 = rec._ksGetRecordCount_2(domain=[])

    def _ksGetRecordCount_2(self, domain=[]):
        rec = self
        if rec.lbs_record_count_type_2 == 'count':
            lbs_record_count = rec.ks_fetch_model_data_2(rec.lbs_model_name_2, rec.lbs_domain_2, 'search_count', rec,
                                                        domain)

        elif rec.lbs_record_count_type_2 in ['sum', 'average'] and rec.record_field_2:
            ks_records_grouped_data = rec.ks_fetch_model_data_2(rec.lbs_model_name_2, rec.lbs_domain_2, 'read_group',
                                                                rec, domain)
            if ks_records_grouped_data and len(ks_records_grouped_data) > 0:
                ks_records_grouped_data = ks_records_grouped_data[0]
                if rec.lbs_record_count_type_2 == 'sum' and ks_records_grouped_data.get('__count', False) and (
                        ks_records_grouped_data.get(rec.record_field_2.name)):
                    lbs_record_count = ks_records_grouped_data.get(rec.record_field_2.name, 0)
                elif rec.lbs_record_count_type_2 == 'average' and ks_records_grouped_data.get(
                        '__count', False) and (ks_records_grouped_data.get(rec.record_field_2.name)):
                    lbs_record_count = ks_records_grouped_data.get(rec.record_field_2.name,
                                                                  0) / ks_records_grouped_data.get('__count',
                                                                                                   1)
                else:
                    lbs_record_count = 0
            ...
            lbs_record_count = False

        return lbs_record_count

    @api.onchange('lbs_model_id_2')
    def make_record_field_empty_2(self):
        for rec in self:
            rec.record_field_2 = False
            rec.lbs_domain_2 = False
            rec.date_filter_field_2 = False
            # To show "created on" by default on date filter field on model select.
            if rec.model_id:
                datetime_field_list = rec.date_filter_field_2.search(
                    [('model_id', '=', rec.model_id.id), '|', ('ttype', '=', 'date'),
                     ('ttype', '=', 'datetime')]).read(['id', 'name'])
                for field in datetime_field_list:
                    if field['name'] == 'create_date':
                        rec.date_filter_field_2 = field['id']
            else:
                rec.date_filter_field_2 = False
                rec.ks_domain_extension_2 = False

    # Writing separate function to fetch dashboard item data
    def ks_fetch_model_data_2(self, lbs_model_name, lbs_domain, ks_func, rec, domain=[]):
        data = 0
        try:
            if lbs_domain and lbs_domain != '[]' and lbs_model_name:
                proper_domain = self.ks_convert_into_proper_domain_2(lbs_domain, rec, domain)
                if ks_func == 'search_count':
                    data = self.env[lbs_model_name].search_count(proper_domain)
                elif ks_func == 'read_group':
                    data = self.env[lbs_model_name].read_group(proper_domain, [rec.record_field_2.name], [],
                                                              lazy=False)
            elif lbs_model_name:
                # Have to put extra if condition here because on load,model giving False value
    ...
    def ks_set_date_filter_2(self):
        for rec in self:
            if (not rec.date_filter_selection_2) or rec.date_filter_selection_2 == "l_none":
                rec.item_start_date_2 = rec.item_end_date = False
            elif rec.date_filter_selection_2 != 'l_custom':
                ks_date_data = ks_get_date(rec.date_filter_selection_2, self, rec.date_filter_field_2.ttype)
                rec.item_start_date_2 = ks_date_data["selected_start_date"]
                rec.item_end_date_2 = ks_date_data["selected_end_date"]

    def ks_convert_into_proper_domain_2(self, lbs_domain_2, rec, domain=[]):
        if lbs_domain_2 and "%UID" in lbs_domain_2:
            lbs_domain_2 = lbs_domain_2.replace('"%UID"', str(self.env.user.id))
        if lbs_domain_2 and "%MYCOMPANY" in lbs_domain_2:
            lbs_domain_2 = lbs_domain_2.replace('"%MYCOMPANY"', str(self.env.company.id))

        ks_date_domain = False

        if rec.date_filter_field_2:
            if not rec.date_filter_selection_2 or rec.date_filter_selection_2 == "l_none":
                selected_start_date = self._context.get('ksDateFilterStartDate', False)
                selected_end_date = self._context.get('ksDateFilterEndDate', False)
                ks_is_def_custom_filter = self._context.get('ksIsDefultCustomDateFilter', False)
                ks_timezone = self._context.get('tz') or self.env.user.tz
                if selected_start_date and selected_end_date and rec.date_filter_field_2.ttype == 'datetime' and not ks_is_def_custom_filter:
                    selected_start_date = ks_convert_into_utc(selected_start_date, ks_timezone)
                    selected_end_date = ks_convert_into_utc(selected_end_date, ks_timezone)
                if selected_start_date and selected_end_date and rec.date_filter_field_2.ttype == 'date' and ks_is_def_custom_filter:
                    selected_start_date = ks_convert_into_local(selected_start_date, ks_timezone)
                    selected_end_date = ks_convert_into_local(selected_end_date, ks_timezone)
                if self._context.get('ksDateFilterSelection', False) and self._context['ksDateFilterSelection'] not in [
                    'l_none', 'l_custom']:
                    ks_date_data = ks_get_date(self._context.get('ksDateFilterSelection'), self,
                                               rec.date_filter_field_2.ttype)
                    selected_start_date = ks_date_data["selected_start_date"]
                    selected_end_date = ks_date_data["selected_end_date"]

                if selected_end_date and not selected_start_date:
                    ks_date_domain = [
                        (rec.date_filter_field_2.name, "<=",
                         selected_end_date.strftime(DEFAULT_SERVER_DATETIME_FORMAT))]
                elif selected_start_date and not selected_end_date:
                    ks_date_domain = [
                        (rec.date_filter_field_2.name, ">=",
                ...
                if rec.date_filter_selection_2 and rec.date_filter_selection_2 != 'l_custom':
                    ks_date_data = ks_get_date(rec.date_filter_selection_2, self, rec.date_filter_field_2.ttype)
                    selected_start_date = ks_date_data["selected_start_date"]
                    selected_end_date = ks_date_data["selected_end_date"]
                else:
                    selected_start_date = False
                    selected_end_date = False
                    if rec.item_start_date_2 or rec.item_end_date_2:
                        selected_start_date = rec.item_start_date_2
                        selected_end_date = rec.item_end_date_2
                        if rec.date_filter_field_2.ttype == 'date' and rec.item_start_date_2 and rec.item_end_date_2:
                            ks_timezone = self._context.get('tz') or self.env.user.tz
                            selected_start_date = ks_convert_into_local(rec.item_start_date_2, ks_timezone)
                            selected_end_date = ks_convert_into_local(rec.item_end_date_2, ks_timezone)
                ...
                if selected_start_date and selected_end_date:
                    ...
                    if rec.ks_year_period_2 and rec.ks_year_period_2 != 0:
                        ...
                        date_field_name = rec.date_filter_field_2.name
                        ...
                        if rec.date_filter_field_2:
                            ...
                            selected_end_date = fields.datetime.strftime(selected_end_date,
                                                                         DEFAULT_SERVER_DATETIME_FORMAT)
                            ks_date_domain = [(rec.date_filter_field_2.name, ">=", selected_start_date),
                                              (rec.date_filter_field_2.name, "<=", selected_end_date)]
                        else:
                            ks_date_domain = []
                elif selected_start_date and rec.date_filter_field_2:
                    selected_start_date = fields.datetime.strftime(selected_start_date, DEFAULT_SERVER_DATETIME_FORMAT)
                    ks_date_domain = [(rec.date_filter_field_2.name, ">=", selected_start_date)]
                elif selected_end_date and rec.date_filter_field_2:
                    selected_end_date = fields.datetime.strftime(selected_end_date, DEFAULT_SERVER_DATETIME_FORMAT)
                    ks_date_domain = [(rec.date_filter_field_2.name, "<=", selected_end_date)]
        else:
            ks_date_domain = []

        proper_domain = safe_eval(lbs_domain_2) if lbs_domain_2 else []
        if ks_date_domain:
            proper_domain.extend(ks_date_domain)
    ...
    def ks_fetch_chart_data(self, lbs_model_name, ks_chart_domain, ks_chart_measure_field_with_type,
                            ks_chart_measure_field_with_type_2,
                            ks_chart_measure_field, ks_chart_measure_field_2,
                            ks_chart_groupby_relation_field, ks_chart_date_groupby, chart_groupby_type, orderby,
        ...
        if chart_groupby_type == "date_type":
        ...
        try:
            if self.ks_fill_temporal and ks_chart_date_groupby not in ['minute', 'hour']:
                ks_chart_records = self.env[lbs_model_name].with_context(fill_temporal=True) \
                ...
                ks_chart_records = self.env[lbs_model_name] \
                    .read_group(ks_chart_domain,
                                list(set(ks_chart_measure_field_with_type + ks_chart_measure_field_with_type_2 +
                                         [ks_chart_groupby_relation_field])), [ks_chart_groupby_field],
                                orderby=orderby, limit=limit, lazy=False)
        ...
        if chart_groupby_type == "relational_type":
        ...
        for res in ks_chart_records:
            ...
            if all(measure_field in res for measure_field in ks_chart_measure_field):
                ...
                elif chart_groupby_type == "selection":
                    selection = res[ks_chart_groupby_field]
                    if selection:
                        label = dict(self.env[lbs_model_name].fields_get(allfields=[ks_chart_groupby_field])
    ...
    def ks_fetch_drill_down_data(self, item_id, domain, sequence):
        ...
        ks_chart_type = action_line.ks_chart_type if action_line.ks_chart_type else record.dashboard_item_type
        ks_list_view_data = {'label': [], 'type': 'grouped',
                             'data_rows': [], 'model': record.lbs_model_name, 'previous_domain': domain, }
        if action_line.ks_chart_type == 'ks_list_view':
            if record.dashboard_item_type == 'ks_list_view':
            ...
            if action_line.ks_item_action_field.ttype == 'many2one':
                ...
                ks_list_view_records = self.env[record.lbs_model_name] \
                ...
                for res in ks_list_view_records:
                    ...
                    if res[ks_selection_field]:
                        data_row['data'].append(dict(
                            self.env[record.lbs_model_name].fields_get(allfields=ks_selection_field)
                            [ks_selection_field]['selection'])[res[ks_selection_field]])
                    else:
                        data_row['data'].append(" ")
                    data_row['ks_column_type'].append(self.lbs_chart_relation_groupby.ttype)
            ...
            ks_chart_data = record.ks_fetch_chart_data(record.lbs_model_name, domain,
                                                       ks_chart_measure_field_with_type,
                                                       ks_chart_measure_field_with_type_2,
                                                       ks_chart_measure_field,
                                                       ks_chart_measure_field_2,
    ...
    def ks_get_start_end_date(self, model_name, ks_chart_groupby_relation_field, ttype, ks_chart_domain,
        ...
        #         self.env['lbs_dashboard.item_goal'].search([('ks_goal_date', '>=', model_field_start_date.strftime("%Y-%m-%d")),
        ...
        goal_model_start_date = \
            self.env['lbs.dashboard_item_goal'].search(ks_goal_domain, limit=1,
                                                            order='ks_goal_date ASC')['ks_goal_date']
        goal_model_end_date = \
            self.env['lbs.dashboard_item_goal'].search(ks_goal_domain, limit=1,
                                                            order='ks_goal_date DESC')['ks_goal_date']
    ...
    def ks_get_next_offset(self, ks_item_id, offset, item_domain=[]):
        ...
        ks_list_domain = self.ks_convert_into_proper_domain(record.lbs_domain, self, item_domain)
    ...
    @api.onchange('lbs_domain_2')
    def ks_onchange_check_domain_2_onchange(self):
        if self.lbs_domain_2:
            proper_domain_2 = []
            try:
                lbs_domain_2 = self.lbs_domain_2
                if "%UID" in lbs_domain_2:
                    lbs_domain_2 = lbs_domain_2.replace("%UID", str(self.env.user.id))
                if "%MYCOMPANY" in lbs_domain_2:
                    lbs_domain_2 = lbs_domain_2.replace("%MYCOMPANY", str(self.env.company.id))
                lbs_domain_2 = safe_eval(lbs_domain_2)

                for element in lbs_domain_2:
                    proper_domain_2.append(element) if type(element) != list else proper_domain_2.append(tuple(element))
                self.env[self.lbs_model_name_2].search_count(proper_domain_2)
            except Exception:
    ...
    def ks_onchange_check_domain_onchange(self):
        if self.lbs_domain:
            proper_domain = []
            try:
                lbs_domain = self.lbs_domain
                if "%UID" in lbs_domain:
                    lbs_domain = lbs_domain.replace("%UID", str(self.env.user.id))
                if "%MYCOMPANY" in lbs_domain:
                    lbs_domain = lbs_domain.replace("%MYCOMPANY", str(self.env.company.id))
                lbs_domain = safe_eval(lbs_domain)
                for element in lbs_domain:
                    proper_domain.append(element) if type(element) != list else proper_domain.append(tuple(element))
                self.env[self.lbs_model_name].search_count(proper_domain)
            except Exception:
                raise UserError("Invalid Domain")


class LBSDashboardItemsGoal(models.Model):
    _name = 'lbs.dashboard_item_goal'
    _description = 'Dashboard Items Goal Lines'

    ks_goal_date = fields.Date(string="Date")
    ks_goal_value = fields.Float(string="Value")

    ks_dashboard_item = fields.Many2one('lbs.dashboard_items', string="Dashboard Item")


class LBSDashboardItemsActions(models.Model):
    _name = 'lbs.dashboard_item_action'
    _description = 'Dashboard Items Action Lines'
    ...
    ks_dashboard_item_id = fields.Many2one('lbs.dashboard_items', string="Dashboard Item")
    model_id = fields.Many2one('ir.model', related='ks_dashboard_item_id.model_id')
...
class LBSDashboardItemMultiplier(models.Model):
    _name = 'lbs.dashboard_items_multiplier'
    _description = 'Dashboard Items Multiplier Lines'
