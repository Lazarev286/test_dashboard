...
class Base(models.AbstractModel):
    ...
    def create(self, vals_list):
        ...
        if 'ir.' not in self._name and self.env.user.has_group('base.group_user'):
            items = self.env['lbs.dashboard_items'].search(
                [['model_id.model', '=', self._name], ['ks_auto_update_type', '=', 'ks_live_update']])
            if items:
                ...
                updates = [[
                    ...
                    {'type': 'lbs_dashboard.notification', 'changes': items.ids},
                    {'id': self.id}
