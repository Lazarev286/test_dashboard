...
class KsDashboardJsonExport(KsDashboardExport, http.Controller):
    ...
    @http.route('/lbs_dashboard/export/dashboard_json', type='http', auth="user")
    @serialize_exception
    def index(self, data):
...
class KsItemJsonExport(KsDashboardExport, http.Controller):
    ...
    @http.route('/lbs_dashboard/export/item_json', type='http', auth="user")
    @serialize_exception
    def index(self, data):
        data = json.loads(data)
        item_id = data["item_id"]
        data['dashboard_data'] = request.env['lbs.dashboard'].ks_export_item(item_id)
