...
class KsListExport(http.Controller):
    ...
    def base(self, data):
        ...
        if ks_export_boolean:
            item = request.env['lbs.dashboard_items'].browse(int(item_id))
            ...
            if item._context.get('ksDateFilterSelection', False):
                date_filter_selection = item._context['ksDateFilterSelection']
                ...
                date_filter_selection = item.dashboard_lbs_board_id.date_filter_selection
                item = item.with_context(ksDateFilterStartDate=item.dashboard_lbs_board_id.dashboard_start_date)
                item = item.with_context(ksDateFilterEndDate=item.dashboard_lbs_board_id.dashboard_end_date)
                item = item.with_context(ksDateFilterSelection=date_filter_selection)
                item = item.with_context(ksIsDefultCustomDateFilter=True)

            if date_filter_selection not in ['l_custom', 'l_none']:
                ks_date_data = ks_get_date(date_filter_selection, request, 'datetime')
            ...
            ks_chart_domain = item.ks_convert_into_proper_domain(item.lbs_domain, item,item_domain)
        ...
        for dataset in list_data['data_rows']:
            ...
            for ks_count, val in enumerate(dataset['data']):
                if isinstance(val, (float, int)):
                    if val >= 0:
                        try:
                            ks_precision = item.sudo().env.ref('lbs_dashboard.ks_dashboard_ninja_precision').digits
                        except Exception as e:
                            ks_precision = 2
...
class KsListExcelExport(KsListExport, http.Controller):
    ...
    @http.route('/lbs_dashboard/export/list_xls', type='http', auth="user")
    @serialize_exception
    def index(self, data):
...
class KsListCsvExport(KsListExport, http.Controller):
    ...
    @http.route('/lbs_dashboard/export/list_csv', type='http', auth="user")
    @serialize_exception
    def index(self, data):
        return self.base(data)
