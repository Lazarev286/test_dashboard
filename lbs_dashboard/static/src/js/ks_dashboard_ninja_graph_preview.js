odoo.define('ks_dashboard_ninja_list.ks_dashboard_graph_preview', function(require) {
    ...
    var KsGraphPreview = AbstractField.extend({
        ...
        _render: function() {
            this.$el.empty()
            if (this.recordData.dashboard_item_type !== 'ks_tile' && this.recordData.dashboard_item_type !== 'ks_kpi' && this.recordData.dashboard_item_type !== 'ks_list_view' && this.recordData.dashboard_item_type !== 'ks_to_do') {
                if (this.recordData.model_id) {
                    if (this.recordData.chart_groupby_type == 'date_type' && !this.recordData.ks_chart_date_groupby) {
                        return this.$el.append($('<div>').text("Select Group by date to create chart based on date groupby"));
                    } else if (this.recordData.ks_chart_data_count_type === "count" && !this.recordData.lbs_chart_relation_groupby) {
                        this.$el.append($('<div>').text("Select Group By to create chart view"));
                    } else if (this.recordData.ks_chart_data_count_type !== "count" && (this.recordData.ks_chart_measure_field.count === 0 || !this.recordData.lbs_chart_relation_groupby)) {
        ...
        _getChartData: function() {
            ...
            else if (field.lbs_model_name) ks_chart_name = field.model_id.data.display_name;
            else ks_chart_name = "Name";

            this.chart_type = this.recordData.dashboard_item_type.split('_')[1];
        ...
        renderChart: function() {
            var self = this;
            if (this.recordData.ks_chart_measure_field_2.count && this.recordData.dashboard_item_type === 'ks_bar_chart') {
                var self = this;
                var scales = {}
                scales.yAxes = [{
                        type: "linear",
