odoo.define('ks_dashboard_ninja_list.ks_dashboard_ninja_list_view_preview', function(require) {
    ...
    var KsListViewPreview = AbstractField.extend({
        ...
        _render: function() {
            ...
            if (rec.dashboard_item_type === 'ks_list_view') {
                if (rec.ks_list_view_type == "ungrouped") {
                    ...
                    if (rec.ks_list_view_group_fields.count !== 0 && rec.lbs_chart_relation_groupby) {
                        if (rec.chart_groupby_type === 'relational_type' || rec.chart_groupby_type === 'selection' || rec.chart_groupby_type === 'other' || rec.chart_groupby_type === 'date_type' && rec.ks_chart_date_groupby) {
                            this.ksRenderListView();
                        } else {
        ...
        ksRenderListView: function() {
            ...
            var count = field.lbs_record_count;
            if (field.name) ks_list_view_name = field.name;
            else if (field.lbs_model_name) ks_list_view_name = field.model_id.data.display_name;
