...
//        Indian format shorthand function
        ...
        _render: function() {
            ...
            self.$el.empty();
            ks_rgba_background_color = self._get_rgba_format(field.background_color)
            ks_rgba_font_color = self._get_rgba_format(field.lbs_font_color)
            ks_rgba_icon_color = self._get_rgba_format(field.default_icon_color)
            item_info = {
                name: field.name,
                //                    count: self.record.specialData.ks_domain.nbRecords.toLocaleString('en', {useGrouping:true}),
                count: self.ksNumFormatter(field.lbs_record_count, 1),
                icon_select: field.icon_select,
                default_icon: field.default_icon,
                icon_color: ks_rgba_icon_color,
                count_tooltip: field_utils.format.float(field.lbs_record_count, Float64Array, {digits: [0, field.ks_precision_digits]}),
            }

            if (field.lbs_icon) {

                if (!utils.is_bin_size(field.lbs_icon)) {
                    // Use magic-word technique for detecting image type
                    item_info['img_src'] = 'data:image/' + (self.file_type_magic_word[field.lbs_icon[0]] || 'png') + ';base64,' + field.lbs_icon;
                } else {
            ...
            if (field.ks_multiplier_active){
                var lbs_record_count = field.lbs_record_count * field.ks_multiplier
                item_info['count'] = self._onKsGlobalFormatter(lbs_record_count, field.ks_data_format, field.ks_precision_digits);
                item_info['count_tooltip'] = lbs_record_count;
            }else{
                item_info['count'] = self._onKsGlobalFormatter(field.lbs_record_count, field.ks_data_format, field.ks_precision_digits);
            }

//            count_tooltip

            switch (field.lbs_layout) {
                case 'layout1':
                ...
                case 'layout2':
                    ...
                    var ks_rgba_dark_background_color_l2 = self._get_rgba_format(self.ks_get_dark_color(field.background_color.split(',')[0], field.background_color.split(',')[1], -10));
        ...
        _onKsGlobalFormatter: function(lbs_record_count, ks_data_format, ks_precision_digits){
            var self = this;
            if (ks_data_format == 'exact'){
//                return lbs_record_count;
                return field_utils.format.float(lbs_record_count, Float64Array, {digits: [0, ks_precision_digits]});
            }else{
                if (ks_data_format == 'indian'){
                    return self.ksNumIndianFormatter( lbs_record_count, 1);
                }else if (ks_data_format == 'colombian'){
                    return self.ksNumColombianFormatter( lbs_record_count, 1, ks_precision_digits);
                }else{
                    return self.ksNumFormatter(lbs_record_count, 1);
                }
            }
        },
