odoo.define('lbs_dashboard.ks_dashboard', function(require) {
    "use strict";

    var core = require('web.core');
    const { patch } = require('web.utils');
    const { WebClient } = require("@web/webclient/webclient");
//    var export = require('lbs_dashboard.import_button');
                ...
                Object.keys(self.ks_dashboard_data.ks_item_data).forEach(function(item_id) {
                    ...
                    if (updateValue) {
                        if (!(item_id in self.ksUpdateDashboard)) {
                            if (['ks_tile', 'ks_list_view', 'ks_kpi', 'ks_to_do'].indexOf(item_data['dashboard_item_type']) >= 0) {
            ...
            return this._super().then(function(){
                self.call('bus_service', 'onNotification', self, function (notifications) {
                    _.each(notifications, (function (notification) {
                        if (notification.hasOwnProperty('type') && notification['type'].type === "lbs_dashboard.notification" && self.ks_mode ==='active') {
                            var item_to_update = _(notification.type.changes).filter((x)=>{return self.ks_dashboard_data.dashboard_items_ids.indexOf(x)>=0});
                            if(item_to_update){
...
//                result = self.normalize_dn_data(result);
        ...
        ks_fetch_items_data: function(){
            var self = this;
            var items_promises = []
            self.ks_dashboard_data.dashboard_items_ids.forEach(function(item_id){
        ...
        ksSortItems: function(ks_item_data) {
            ...
            if (self.ks_dashboard_data.gridstack_config) {
                self.gridstackConfig = JSON.parse(self.ks_dashboard_data.gridstack_config);
        ...
        ksRenderDashboardItems: function(items) {
            ...
            var ks_container_class = 'grid-stack-item',
                ...
                if (self.grid) {
                    ...
                    if (items[i].dashboard_item_type === 'ks_tile') {
...
//                            self.grid.addWidget($(item_view), self.gridstackConfig[items[i].id].x, self.gridstackConfig[items[i].id].y, self.gridstackConfig[items[i].id].width, self.gridstackConfig[items[i].id].height, false, 6, null, 2, 2, items[i].id);
                    ...
                    } else if (items[i].dashboard_item_type === 'ks_list_view') {
                        self._renderListView(items[i], self.grid)
                    }else if (items[i].dashboard_item_type === 'ks_kpi') {
        ...
        _ksRenderDashboardTile: function(tile) {
            ...
            if (tile.ks_multiplier_active){
                var lbs_record_count = tile.lbs_record_count * tile.ks_multiplier
                var data_count = KsGlobalFunction._onKsGlobalFormatter(lbs_record_count, tile.data_formatting, tile.ks_precision_digits);
                var count = lbs_record_count;
            }else{
                 var data_count = KsGlobalFunction._onKsGlobalFormatter(tile.lbs_record_count, tile.data_formatting, tile.ks_precision_digits);
                 var count = lbs_record_count
            }
            if (tile.icon_select == "Custom") {
                if (tile.lbs_icon[0]) {
                    ks_icon_url = 'data:image/' + (self.file_type_magic_word[tile.lbs_icon[0]] || 'png') + ';base64,' + tile.lbs_icon;
                } else {
                    ks_icon_url = false;
                }
            }
            ...
            ks_rgba_background_color = self._ks_get_rgba_format(tile.background_color);
            ks_rgba_font_color = self._ks_get_rgba_format(tile.lbs_font_color);
            ks_rgba_default_icon_color = self._ks_get_rgba_format(tile.default_icon_color);
            ...
            switch (tile.lbs_layout) {
                ...
                case 'layout2':
                    var ks_rgba_dark_background_color_l2 = self._ks_get_rgba_format(self.ks_get_dark_color(tile.background_color.split(',')[0], tile.background_color.split(',')[1], -10));
        ...
        _renderChart: function($ks_gridstack_container, item) {
            ...
            if (chart_data["ks_show_second_y_scale"] && item.dashboard_item_type === 'ks_bar_chart') {
                var scales = {}
                scales.yAxes = [{
                        type: "linear",
                        display: true,
                        ...
                        ticks: {
                            ...
                            callback: function(value, index, values) {
                                ...
                                if (ks_selection === 'monetary') {
                                    ...
                                    var ks_data = value;
                                    ks_data = KsGlobalFunction._onKsGlobalFormatter(ks_data, item.data_formatting, item.ks_precision_digits);
                                    ks_data = KsGlobalFunction.ks_monetary(ks_data, ks_currency_id);
                                   return ks_data;
                                } else if (ks_selection === 'custom') {
                                    var ks_field = chart_data.ks_field;
                                    return KsGlobalFunction._onKsGlobalFormatter(value, item.data_formatting, item.ks_precision_digits) + ' ' + ks_field;

                                } else {
                                   return KsGlobalFunction._onKsGlobalFormatter(value, item.data_formatting, item.ks_precision_digits);
                                }
                            },
                        }
                    }
                        ...
                        if (item_data.max_sequnce != 0 && sequnce < item_data.max_sequnce) {
                            self._rpc({
                                ...
                                if (result.ks_chart_data) {
                                    self.ks_dashboard_data.ks_item_data[item_id]['dashboard_item_type'] = result.ks_chart_type;
                                    ...
                                    self.ks_dashboard_data.ks_item_data[item_id]['dashboard_item_type'] = 'ks_list_view';
                ...
                if ('domains' in item_data) {
                    ...
                    if (sequnce >= 0) {
                        self._rpc({
                            ...
                            self.ks_dashboard_data.ks_item_data[item_id]['ks_chart_data'] = result.ks_chart_data;
                            self.ks_dashboard_data.ks_item_data[item_id]['sequnce'] = result.sequence;
                            if (result.ks_chart_type)  self.ks_dashboard_data.ks_item_data[item_id]['dashboard_item_type'] = result.ks_chart_type;
                            $(self.$el.find(".grid-stack-item[gs-id=" + item_id + "]").children()[0]).find(".ks_dashboard_item_drill_up").removeClass('d-none');
                            $(self.$el.find(".grid-stack-item[gs-id=" + item_id + "]").children()[0]).find(".card-body").empty();
                            if (result.ks_chart_data) {
                                var item_data = self.ks_dashboard_data.ks_item_data[item_id];
...
//                        $(self.$el.find('.ks_pager')).removeClass('d-none');
            ...
            return self._rpc({
                ...
                if (item_data.ks_list_view_data) {
                    ...
                    if (new_item_data["ks_list_view_type"] === "ungrouped" && JSON.parse(item_data['ks_list_view_data']).data_rows.length) {
                        ...
                        if (item.lbs_record_count <= item.ks_pagination_limit) item_view.find('.ks_load_next').addClass('ks_event_offer_list');
        ...
        onChartMoreInfoClick: function(evt) {
            ...
            var item_data = self.ks_dashboard_data.ks_item_data[item_id];
            var groupBy = item_data.chart_groupby_type === 'relational_type' ? item_data.ks_chart_relation_groupby_name : item_data.ks_chart_relation_groupby_name + ':' + item_data.ks_chart_date_groupby;
            ...
            if (e.currentTarget.dataset.item !== "ks_json") {
                self.do_action({
                    ...
                    context: {
                        ...
                        'set_interval': self.ks_dashboard_data.set_interval,
                        'data_formatting':self.ks_dashboard_data.data_formatting,
                    },
                }, {
...
//                            framework.unblockUI();
            ...
            if (dashboard_title ==="") {
                ...
                if (!self.ks_dashboard_data.ks_child_boards){
                    self.ks_dashboard_data.ks_child_boards = {
                        'ks_default': [this.ks_dashboard_data.name, self.ks_dashboard_data.gridstack_config]
                    }
                }
                ...
                this._rpc({
                    ...
                    args: ['create', self.ks_dashboard_id, {
                        ...
                        "dashboard_ninja_id": self.ks_dashboard_id,
                        "name": dashboard_title,
                        "is_active": true,
        ...
        ksUpdateDashboardItem: function(ids) {
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (item_data['dashboard_item_type'] == 'ks_list_view') {
                    ...
                    if (ks_length) {
                        if (item_view.find('.ks_pager_name')) {
                            ...
                            if (item_data.ks_record_data_limit == item_data.ks_pagination_limit || item_data.lbs_record_count==item_data.ks_pagination_limit) {
                                item_view.find('.ks_load_next').addClass('ks_event_offer_list');
                            }
                    } else {
                        item_view.find('.ks_pager').addClass('d-none');
                    }
                } else if (item_data['dashboard_item_type'] == 'ks_tile') {
                ...
                } else if (item_data['dashboard_item_type'] == 'ks_kpi') {
       ...
       var self = this;
        ...
        _ksSaveCurrentLayout: function() {
            ...
            self.ks_dashboard_data.gridstack_config = JSON.stringify(grid_config);
            if(this.ks_dashboard_data.ks_selected_board_id && this.ks_dashboard_data.ks_child_boards){
        ...
        ks_get_current_gridstack_config: function(){
            ...
            if (self.ks_dashboard_data.gridstack_config) {
                _.extend(grid_config, JSON.parse(self.ks_dashboard_data.gridstack_config))
            }
            if (items){
                for (var i = 0; i < items.length; i++) {
        ...
        ksSum: function(count_1, count_2, item_info, field, target_1, $kpi_preview, kpi_data) {
            ...
            if (field.ks_multiplier_active){
                item_info['count'] = KsGlobalFunction._onKsGlobalFormatter(count* field.ks_multiplier, field.data_formatting, field.ks_precision_digits);
                ...
                item_info['count'] = KsGlobalFunction._onKsGlobalFormatter(count, field.data_formatting, field.ks_precision_digits);
        ...
        renderKpi: function(item, grid) {
            ...
            if (field.date_filter_selection === "l_none") date_filter_selection = self.ks_dashboard_data.date_filter_selection;
            ...
            var ks_rgba_background_color = self._ks_get_rgba_format(field.background_color);
            var ks_rgba_button_color = self._ks_get_rgba_format(field.ks_button_color);
            var ks_rgba_font_color = self._ks_get_rgba_format(field.lbs_font_color);
            ...
            if (field.previous_period && ks_valid_date_selection.indexOf(date_filter_selection) >= 0) {
            ...
            if (field.icon_select == "Custom") {
                if (field.lbs_icon[0]) {
                    ks_icon_url = 'data:image/' + (self.file_type_magic_word[field.lbs_icon[0]] || 'png') + ';base64,' + field.lbs_icon;
                } else {
                    ks_icon_url = false;
                }
            }
//            parseInt(Math.round((count_1 / target_1) * 100)) ? field_utils.format.integer(Math.round((count_1 / target_1) * 100)) : "0"
            ...
            var ks_rgba_icon_color = self._ks_get_rgba_format(field.default_icon_color)
            var item_info = {
                item: item,
                id: field.id,
                ...
                previous_period: ks_valid_date_selection.indexOf(date_filter_selection) >= 0 ? field.previous_period : false,
            ...
            if (field.ks_multiplier_active){
                item_info['count_1'] = KsGlobalFunction._onKsGlobalFormatter(kpi_data[0]['record_data'] * field.ks_multiplier, field.data_formatting, field.ks_precision_digits);
                item_info['count_1_tooltip'] = kpi_data[0]['record_data'] * field.ks_multiplier
            }else{
                item_info['count_1'] = KsGlobalFunction._onKsGlobalFormatter(kpi_data[0]['record_data'], field.data_formatting, field.ks_precision_digits);
            }
            item_info['target'] = KsGlobalFunction._onKsGlobalFormatter(kpi_data[0].target, field.data_formatting, field.ks_precision_digits);
            var $kpi_preview;
            if (!kpi_data[1]) {
                ...
                if (field.previous_period && String(previous_period_data) && ks_valid_date_selection.indexOf(date_filter_selection) >= 0) {
                    if (pre_acheive) {
                ...
                switch (field.ks_data_comparison) {
                    case "None":
                        if (field.ks_multiplier_active){
                            ...
                            item_info['count'] = String(KsGlobalFunction._onKsGlobalFormatter(count_1 * field.ks_multiplier, field.data_formatting, field.ks_precision_digits)) + "/" + String(KsGlobalFunction._onKsGlobalFormatter(count_2 * field.ks_multiplier, field.data_formatting, field.ks_precision_digits));
                         }else{
                            ...
                            item_info['count'] = String(KsGlobalFunction._onKsGlobalFormatter(count_1, field.data_formatting, field.ks_precision_digits)) + "/" + String(KsGlobalFunction._onKsGlobalFormatter(count_2, field.data_formatting, field.ks_precision_digits));
                         }
        ...
        _KsGetDateValues: function() {
            ...
            var date_filter_selected = self.ks_dashboard_data.date_filter_selection;
...
//                    $('.ks_date_input_fields').addClass("ks_hide");
            ...
            self.$el.find('#date_filter_selection').text(self.ks_date_filter_selections[date_filter_selected]);

            if (self.ks_dashboard_data.date_filter_selection === 'l_custom') {
            ...
            } else if (self.ks_dashboard_data.date_filter_selection !== 'l_custom') {
...
//                  self.ksUpdateDashboardItem()
        ...
        _renderDateFilterDatePicker: function() {
            var self = this;
            self.$el.find(".ks_dashboard_link").removeClass("ks_hide");
            var startDate = self.ks_dashboard_data.dashboard_start_date ? moment.utc(self.ks_dashboard_data.dashboard_start_date).local() : moment();
            var endDate = self.ks_dashboard_data.dashboard_end_date ? moment.utc(self.ks_dashboard_data.dashboard_end_date).local() : moment();
        ...
        _ksOnDateFilterMenuSelect: function(e) {
            if (e.target.id !== 'ks_date_selector_container') {
                ...
                $('#date_filter_selection').text(self.ks_date_filter_selections[e.target.parentElement.id]);
        ...
        ksChartExportXlsCsv: function(e) {
            ...
            if (this.ks_dashboard_data.ks_item_data[chart_id].dashboard_item_type === 'ks_list_view'){
