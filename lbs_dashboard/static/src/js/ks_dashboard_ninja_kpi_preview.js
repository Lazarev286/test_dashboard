odoo.define('ks_dashboard_ninja_list.ks_dashboard_kpi_preview', function(require) {
    ...
    var KsKpiPreview = AbstractField.extend({
        ...
        _render: function() {
            this.$el.empty();
            if (this.recordData.model_id && this.recordData.dashboard_item_type === "ks_kpi") {
                if (!this.recordData.lbs_model_id_2) {
                    if (!(this.recordData.record_count_type === 'count')) {
                        if (this.recordData.lbs_record_field) {
                            this.renderKpi();
                        } else {
                    ...
                    if (!(this.recordData.lbs_record_count_type_2 === 'count') && !(this.recordData.record_count_type === 'count')) {
                        if (this.recordData.record_field_2 && this.recordData.lbs_record_field) {
                            this.renderKpi();
                        } else {
                            this.$el.append("Select a Record fields ")
                        }
                    } else if (!(this.recordData.lbs_record_count_type_2 === 'count') && (this.recordData.record_count_type === 'count')) {
                        if (this.recordData.record_field_2) {
                            this.renderKpi();
                        } else {
                            this.$el.append("Select a Record field")
                        }
...
//            }
            ...
            var ks_rgba_background_color = self._get_rgba_format(field.background_color);
            var ks_rgba_font_color = self._get_rgba_format(field.lbs_font_color)
            ...
            if (field.previous_period && ks_valid_date_selection.indexOf(field.date_filter_selection) >= 0) {
            ...
            var ks_rgba_icon_color = self._get_rgba_format(field.default_icon_color)
            var item_info = {
                ...
                previous_period: ks_valid_date_selection.indexOf(field.date_filter_selection) >= 0 ? field.previous_period : false,
            ...
            if (!kpi_data[1]) {
                ...
                if (field.previous_period && String(previous_period_data) && ks_valid_date_selection.indexOf(field.date_filter_selection) >= 0) {
                    if (pre_acheive) {
